...
public class LoggerAdapter<TClass> : ILoggerAdapter
{
    ...
    /// <typeparam name="TClass"></typeparam>
    public LoggerAdapter(ILogger<TClass> logger) =>
        _logger = logger;
    ...
    public IDisposable GetLoggerScope(IHandler handler, string methodName) =>
        _logger?.BeginScope(new Dictionary<string, object>
        {
            ...
            { "Seq-Stopwatch", handler?.Configuration.Sequence.Stopwatch.Elapsed ?? TimeSpan.Zero },
            { "Method", methodName },
        });
